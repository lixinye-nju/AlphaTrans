...
class CSVFormatTest(unittest.TestCase):
    ...
    def testWithEscapeCRThrowsExceptions(self) -> None:
        ...
        with self.assertRaises(ValueError):
            CSVFormat.DEFAULT.withEscape0(Constants.CR)
    ...
    def testWithCommentStartCRThrowsException(self) -> None:
        ...
        with pytest.raises(ValueError):
            CSVFormat.DEFAULT.withCommentMarker0(Constants.CR)
    ...
    def testQuoteCharSameAsCommentStartThrowsExceptionForWrapperType_Deprecated(
        ...
        with pytest.raises(ValueError):
            CSVFormat.DEFAULT.withQuote1("!").withCommentMarker0('!')
    ...
    def testFormatThrowsNullPointerException(self) -> None:
        ...
        with pytest.raises(TypeError) as e:
            csv_format.format_(None)
        
        self.assertIn("NoneType", str(e.value))
    ...
    def testEscapeSameAsCommentStartThrowsExceptionForWrapperType_Deprecated(
        ...
        with pytest.raises(ValueError):
            CSVFormat.DEFAULT.withEscape1("!").withCommentMarker1(
                "!"
            )
    ...
    def __assertNotEquals1(
        ...
        if left == right or right == left:
            self.fail("Objects must not compare equal for " + str(name) + "(" + str(type_) + ")")
        if hash(left) == hash(right):
            self.fail("Hash code should not be equal for " + str(name) + "(" + str(type_) + ")")
