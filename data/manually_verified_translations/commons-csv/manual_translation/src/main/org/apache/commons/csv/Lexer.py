...
class Lexer:
    ...
    def trimTrailingSpaces(self, buffer: typing.union[str, io.StringIO]) -> None:

        if isinstance(buffer, io.StringIO):
            bufferContent = buffer.getvalue()
            strippedContent = bufferContent.rstrip(' \t\n\r\x0b\x0c\xa0')
            if strippedContent != bufferContent:
                buffer.seek(0)
                buffer.truncate(0)
                buffer.write(strippedContent)
                buffer.seek(0)
        else:
            raise TypeError("buffer must be of type `io.StringIO` to be modified in-place.")
    ...
    def readEscape(self) -> int:
        ...
        if ch == ord(Constants.CR) or ch == ord(Constants.LF) or ch == ord(Constants.FF)\
            or ch == ord(Constants.TAB) or ch == ord(Constants.BACKSPACE):
            return ch
