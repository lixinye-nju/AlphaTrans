...
class ISBNValidatorTest(unittest.TestCase):
    ...
    __invalidISBN10Format: typing.List[typing.List[str]] = [
        [""],  # empty
        ["   "],  # empty
        ["1"],  # too short
        ["123456789"],  # too short
    ...
    def testInvalidISBN10Format(self) -> None:
        ...
        for i, code in enumerate(self.__invalidISBN10Format):
            self.assertFalse(f"Pattern[{i}]={code}", pattern.match(code) is not None)
            self.assertFalse(
                f"isValidISBN10[{i}]={code}", validator.isValidISBN10(code)
            )
            self.assertIsNone(
                f"validateISBN10[{i}]={code}", validator.validateISBN10(code)
            )
