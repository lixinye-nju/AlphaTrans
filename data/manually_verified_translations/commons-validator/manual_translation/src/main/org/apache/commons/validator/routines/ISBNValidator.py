...
class ISBNValidator:
    ...
    ISBN13_REGEX: str = "^(978|979)(?:(\\d{10})|(?:" +\
        __SEP + __GROUP + __SEP + __PUBLISHER + __SEP + __TITLE + __SEP +\
        "([0-9])))$"

    ISBN10_REGEX: str = "^(?:(\\d{9}[0-9X])|(?:" +\
        __GROUP + __SEP + __PUBLISHER + __SEP + __TITLE + __SEP +\
        "([0-9X])))$"

    __isbn13Validator: CodeValidator = CodeValidator.CodeValidator4(ISBN13_REGEX, 13, EAN13CheckDigit.EAN13_CHECK_DIGIT)
    __isbn10Validator: CodeValidator = CodeValidator.CodeValidator4(ISBN10_REGEX, 10, ISBN10CheckDigit.ISBN10_CHECK_DIGIT)
