...
class AbstractCalendarValidatorTest(unittest.TestCase, ABC):
    ...
    def testFormat(self) -> None:
        ...
        self.assertEqual(
            self._validator._format1(test, "dd.MM.yy"), "28.11.05", "Format pattern"
        )
        self.assertEqual(
            self._validator._format2(test, "en_US.UTF-8"), "11/28/05", "Format locale"
        )
    ...
    def testLocaleInvalid(self) -> None:
        ...
        for i, locale in enumerate(self._localeInvalid):
            ...
            date = self._validator._parse(locale, None, "en_US.UTF-8", None)
            self.assertIsNone(date, f"validateObj() {text}{date}")
            self.assertFalse(
                self._validator.isValid2(locale, "en_US.UTF-8"), f"isValid() {text}"
            )
    ...
    def testLocaleValid(self) -> None:
        ...
        for i in range(len(self._localeValid)):
            ...
            date = self._validator._parse(self._localeValid[i], None, "en_US.UTF-8", None)
            self.assertIsNotNone(date, "validateObj() " + text + str(date))
            self.assertTrue(
                self._validator.isValid2(self._localeValid[i], "en_US.UTF-8"),
                "isValid() " + text,
            )
