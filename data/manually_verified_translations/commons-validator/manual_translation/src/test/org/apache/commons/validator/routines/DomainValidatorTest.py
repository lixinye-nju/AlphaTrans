...
class DomainValidatorTest(unittest.TestCase):
    ...
    def testDomainNoDots(self) -> None:
        ...
        self.assertTrue(
            validator.isValidDomainSyntax("a"), "a (alpha) should validate"
        )
        self.assertTrue(
            validator.isValidDomainSyntax("9"), "9 (alphanum) should validate"
        )
        self.assertTrue(
            validator.isValidDomainSyntax("c-z"),
            "c-z (alpha - alpha) should validate",
        )

        self.assertFalse(
            validator.isValidDomainSyntax("c-"), "c- (alpha -) should fail"
        )
        self.assertFalse(
            validator.isValidDomainSyntax("-c"), "-c (- alpha) should fail"
        )
        self.assertFalse(validator.isValidDomainSyntax("-"), "- (-) should fail")
    ...
    def testTopLevelDomains(self) -> None:
        ...
        self.assertTrue(
            validator.isValidInfrastructureTld(".arpa"),
            ".arpa should validate as iTLD",
        )
        self.assertFalse(
            validator.isValidInfrastructureTld(".com"),
            ".com shouldn't validate as iTLD",
        )

        self.assertTrue(
            validator.isValidGenericTld(".name"), ".name should validate as gTLD"
        )
        self.assertFalse(
            validator.isValidGenericTld(".us"), ".us shouldn't validate as gTLD"
        )

        self.assertTrue(
            validator.isValidCountryCodeTld(".uk"), ".uk should validate as ccTLD"
        )
        self.assertFalse(
            validator.isValidCountryCodeTld(".org"),
            ".org shouldn't validate as ccTLD",
        )

        self.assertTrue(
            validator.isValidTld(".COM"), ".COM should validate as TLD"
        )
        self.assertTrue(
            validator.isValidTld(".BiZ"), ".BiZ should validate as TLD"
        )

        self.assertFalse(
            validator.isValid(".nope"), "invalid TLD shouldn't validate"
        )
        self.assertFalse(
            validator.isValid(""), "empty string shouldn't validate as TLD"
        )
        self.assertFalse(validator.isValid(None), "null shouldn't validate as TLD")
    ...
    def testInvalidDomains(self) -> None:
        ...
        self.assertFalse(
            validator.isValid(".org"), "bare TLD .org shouldn't validate"
        )
        self.assertFalse(
            validator.isValid(" apache.org "),
            "domain name with spaces shouldn't validate",
        )
        self.assertFalse(
        ...
        self.assertFalse(
            validator.isValid("apache.rog"),
            "domain name with invalid TLD shouldn't validate",
        )

        self.assertFalse(
            validator.isValid("http://www.apache.org"), "URL shouldn't validate"
        )
        self.assertFalse(
            validator.isValid(" "),
        ...
        self.assertFalse(
            validator.isValid(None), "Null shouldn't validate as domain name"
        )
    ...
    def __isSortedLowerCase0(arrayName: str) -> bool:
        ...
        try:
            return DomainValidatorTest.__isSortedLowerCase1(arrayName, f)
        finally:
            pass
