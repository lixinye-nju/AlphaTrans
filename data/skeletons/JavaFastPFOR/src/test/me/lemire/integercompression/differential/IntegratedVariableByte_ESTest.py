from __future__ import annotations

# Imports Begin
# from src.main.org.evosuite.runtime.EvoRunnerParameters import *
# from src.main.org.evosuite.runtime.EvoAssertions import *
# from src.test.me.lemire.integercompression.differential.IntegratedVariableByte_ESTest_scaffolding import *
from src.main.me.lemire.integercompression.differential.IntegratedVariableByte import *
from src.main.me.lemire.integercompression.IntWrapper import *
import unittest
import io

# Imports End


class IntegratedVariableByte_ESTest(unittest.TestCase):

    # Class Fields Begin
    # Class Fields End

    # Class Methods Begin
    def test41(self) -> None:
        pass

    def test40(self) -> None:
        pass

    def test39(self) -> None:
        pass

    def test38(self) -> None:
        pass

    def test37(self) -> None:
        pass

    def test36(self) -> None:
        pass

    def test35(self) -> None:
        pass

    def test34(self) -> None:
        pass

    def test33(self) -> None:
        pass

    def test32(self) -> None:
        pass

    def test31(self) -> None:
        pass

    def test30(self) -> None:
        pass

    def test29(self) -> None:
        pass

    def test28(self) -> None:
        pass

    def test27(self) -> None:
        pass

    def test26(self) -> None:
        pass

    def test25(self) -> None:
        pass

    def test24(self) -> None:
        pass

    def test23(self) -> None:
        pass

    def test22(self) -> None:
        pass

    def test21(self) -> None:
        pass

    def test20(self) -> None:
        pass

    def test19(self) -> None:
        pass

    def test18(self) -> None:
        pass

    def test17(self) -> None:
        pass

    def test16(self) -> None:
        pass

    def test15(self) -> None:
        pass

    def test14(self) -> None:
        pass

    def test13(self) -> None:
        pass

    def test12(self) -> None:
        pass

    def test11(self) -> None:
        pass

    def test10(self) -> None:
        pass

    def test09(self) -> None:
        pass

    def test08(self) -> None:
        pass

    def test07(self) -> None:
        pass

    def test06(self) -> None:
        pass

    def test05(self) -> None:
        pass

    def test04(self) -> None:
        pass

    def test03(self) -> None:
        pass

    def test02(self) -> None:
        pass

    def test01(self) -> None:
        pass

    def test00(self) -> None:
        pass

    # Class Methods End
